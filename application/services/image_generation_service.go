...
func normalizeCharacterFrameType(frameType *string) string {
	if frameType == nil {
		return "sheet"
	}

	switch strings.ToLower(strings.TrimSpace(*frameType)) {
	case "sheet", "turnaround", "turnaround_sheet", "baseline_sheet":
		return "sheet"
	case "front", "first", "main", "":
		return "front"
	case "side", "profile":
		return "side"
	case "back", "rear":
		return "back"
	default:
		return strings.ToLower(strings.TrimSpace(*frameType))
	}
}

func buildCharacterSheetReferenceImages(imageURL string) ([]byte, error) {
	references := []string{}
...
var (
	storyboardFrameHintPattern = regexp.MustCompile(`(?i)(anime\s+style|first\s+frame|key\s+frame|last\s+frame|action\s+frame|panel\s+frame|image\s*ratio\s*:\s*\d+\s*:\s*\d+)`)
	duplicateCommaPattern      = regexp.MustCompile(`[，,]\s*[，,]+`)
	duplicateWhitespacePattern = regexp.MustCompile(`\s{2,}`)
)
...
// sanitizeSensitivePrompt 替换容易触发上游安全策略的敏感词
func sanitizeSensitivePrompt(prompt string) (string, []string) {
	rules := []sensitiveRule{
		{from: "电击爪", to: "机械爪"},
		{from: "武器", to: "工具"},
		{from: "枪", to: "器械"},
	...
	for _, rule := range rules {
		if strings.Contains(prompt, rule.from) {
			prompt = strings.ReplaceAll(prompt, rule.from, rule.to)
			replacements = append(replacements, fmt.Sprintf("%s->%s", rule.from, rule.to))
		}
	}
	return prompt, replacements
}

func sanitizeStoryboardPrompt(prompt string) string {
	cleaned := strings.TrimSpace(prompt)
	if cleaned == "" {
		return cleaned
	}

	cleaned = storyboardFrameHintPattern.ReplaceAllString(cleaned, " ")
	cleaned = duplicateCommaPattern.ReplaceAllString(cleaned, ", ")
	cleaned = duplicateWhitespacePattern.ReplaceAllString(cleaned, " ")
...
func sanitizeMinorStoryboardPrompt(prompt string) string {
	...
	safePrompt += "，整体画面保持健康、克制、非伤害性的叙事表达。"
	return safePrompt
}

func mergeCharacterReferenceImages(raw []byte, frameType string, imageURL string) ([]byte, error) {
	viewOrder := []string{"front", "side", "back"}
	existing := make([]string, len(viewOrder))
	if len(raw) > 0 {
		var decoded []string
		_ = json.Unmarshal(raw, &decoded)
		for index := 0; index < len(viewOrder) && index < len(decoded); index++ {
			existing[index] = strings.TrimSpace(decoded[index])
		}
	}
	...
	if index, ok := frameIndex[frameType]; ok {
		existing[index] = strings.TrimSpace(imageURL)
	}
...
func (s *ImageGenerationService) ProcessImageGeneration(imageGenID uint) {
	...
	// 构建完整的提示词：统一项目风格，移除分镜里遗留的冲突风格后缀
	prompt := imageGen.Prompt
	if imageGen.ImageType == string(models.ImageTypeStoryboard) {
		prompt = sanitizeStoryboardPrompt(prompt)
		prompt = sanitizeMinorStoryboardPrompt(prompt)
	}
	if sanitized, replacements := sanitizeSensitivePrompt(prompt); len(replacements) > 0 {
		s.log.Infow("Prompt sanitized for sensitive terms", "id", imageGenID, "replacements", replacements)
		prompt = sanitized
	}
...
func (s *ImageGenerationService) completeImageGeneration(imageGenID uint, result *image.ImageResult) {
	...
	if imageGen.CharacterID != nil {
		...
		if err := s.db.Select("id", "reference_images").Where("id = ?", *imageGen.CharacterID).First(&character).Error; err != nil {
			s.log.Errorw("Failed to load character before updating generated image", "error", err, "character_id", *imageGen.CharacterID)
		}

		characterUpdates := map[string]interface{}{}
		frameType := normalizeCharacterFrameType(imageGen.FrameType)
		if frameType == "sheet" || frameType == "front" {
			characterUpdates["image_url"] = result.ImageURL
			if localPath != nil {
		...
		if frameType == "sheet" {
			if referenceImages, err := buildCharacterSheetReferenceImages(result.ImageURL); err != nil {
				s.log.Errorw("Failed to build character sheet reference images", "error", err, "character_id", *imageGen.CharacterID)
			} else {
				characterUpdates["reference_images"] = referenceImages
			}
		} else {
			if mergedReferenceImages, err := mergeCharacterReferenceImages(character.ReferenceImages, frameType, result.ImageURL); err != nil {
				s.log.Errorw("Failed to merge character reference images", "error", err, "character_id", *imageGen.CharacterID, "frame_type", frameType)
			} else {
				characterUpdates["reference_images"] = mergedReferenceImages
			}
		}

		if len(characterUpdates) > 0 {
...
func (s *ImageGenerationService) getImageClient(provider string) (image.ImageClient, error) {
	config, err := s.aiService.GetPreferredConfig("image", provider, "")
	if err != nil {
	...
	case "openai", "dalle", "chatfire", "openclaw", "custom_relay":
...
func (s *ImageGenerationService) getImageClientWithModel(provider string, modelName string) (image.ImageClient, error) {
	...
	config, err = s.aiService.GetPreferredConfig("image", provider, modelName)
	if err != nil {
		return nil, fmt.Errorf("no image AI config found: %w", err)
	}
...
func preferredStoryboardsForGeneration(storyboards []models.Storyboard) []models.Storyboard {
	primary := make([]models.Storyboard, 0, len(storyboards))
	for _, storyboard := range storyboards {
		if storyboard.IsPrimary {
			primary = append(primary, storyboard)
		}
	}
	if len(primary) > 0 {
		return primary
...
func (s *ImageGenerationService) BatchGenerateImagesForEpisode(episodeID string) ([]*models.ImageGeneration, error) {
	...
	if err := s.db.Where("episode_id = ?", episodeID).Order("storyboard_number ASC").Find(&scenes).Error; err != nil {
		return nil, fmt.Errorf("failed to get scenes: %w", err)
	}
	targetStoryboards := preferredStoryboardsForGeneration(scenes)

	backgrounds := s.extractUniqueBackgrounds(targetStoryboards)
	s.log.Infow("Extracted unique backgrounds",
		"episode_id", episodeID,
		"background_count", len(backgrounds),
		"target_storyboard_count", len(targetStoryboards))

	// 为每个背景生成图片
	var results []*models.ImageGeneration
	for _, bg := range targetStoryboards {
