...
func (s *StoryboardService) GenerateStoryboard(episodeID string, model string) (string, error) {
	...
	if episode.ScriptContent != nil && *episode.ScriptContent != "" {
        ...
        return "", fmt.Errorf("script content is empty, please generate episode content first")
	}
...
**duration时长估算规则（秒）**：
- **所有镜头时长必须在12-18秒范围内**，优先保持单镜头接近15秒，保证镜头更完整。
...
4. **最终时长** = 基础时长 + 对话调整 + 动作调整，确保结果在12-18秒范围内，并优先接近15秒
