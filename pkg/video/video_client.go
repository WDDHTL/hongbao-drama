...
func buildDoubaoSafePrompt(prompt string, options *VideoOptions) string {
	...
	const maxPromptRunes = 650
	if utf8.RuneCountInString(cleaned) > maxPromptRunes {
		runes := []rune(cleaned)
		cleaned = strings.TrimSpace(string(runes[:maxPromptRunes]))
	}

	suffix := make([]string, 0, 2)
	if options.Duration > 0 {
		suffix = append(suffix, fmt.Sprintf("视频时长约 %d 秒", options.Duration))
	}
	if options.AspectRatio != "" && options.AspectRatio != "adaptive" {
		suffix = append(suffix, fmt.Sprintf("画面比例 %s", options.AspectRatio))
	}

	if len(suffix) > 0 {
		if cleaned != "" && !strings.HasSuffix(cleaned, "。") && !strings.HasSuffix(cleaned, ".") {
			cleaned += "。"
		}
		cleaned += strings.Join(suffix, "，") + "。"
	}
