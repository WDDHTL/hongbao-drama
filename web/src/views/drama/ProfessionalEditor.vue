...
// 将已完成的视频生成结果同步到素材库（去重）
const syncCompletedVideosToAssets = async () => {
  if (!generatedVideos.value.length) return;

  const needImport = generatedVideos.value.filter(
    (video) =>
      video.status === "completed" &&
      video.video_url &&
      !videoAssets.value.some(
        (asset) =>
          asset.video_gen_id === video.id ||
          asset.url === video.video_url ||
          (video.storyboard_id &&
            asset.storyboard_id === video.storyboard_id),
      ),
  );

  if (!needImport.length) return;

  for (const video of needImport) {
    try {
      await assetAPI.importFromVideo(video.id);
    } catch (error) {
      console.error("自动导入视频到素材库失败:", error);
    }
  }
...
const loadStoryboardVideos = async (storyboardId: number) => {
  ...
  try {
    ...
    // 首次加载时，把已完成但未入库的视频同步到素材库
    await syncCompletedVideosToAssets();
