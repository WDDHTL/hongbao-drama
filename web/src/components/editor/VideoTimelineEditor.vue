<template>
  <div class="video-timeline-editor">
      ...
      <div class="preview-panel">
        <div class="video-preview" @click="togglePlay">
          ...
          <div
            ...
            :class="[
              `transition-${getTransitionPreviewType(transitionState.type)}`,
              {
    ...
    <el-dialog v-model="transitionDialogVisible" title="设置转场效果" width="500px">
      <el-form class="transition-form" label-width="100px">
        <el-form-item :label="$t('video.transitionType')">
          <el-select
            v-model="selectedTransitionPresetValue"
            :placeholder="$t('video.selectTransition')"
            filterable
            @change="handleTransitionPresetChange"
          >
            ...
            <el-option label="像素化" value="pixelize" />
            <el-option label="自定义添加..." :value="CUSTOM_TRANSITION_VALUE" />
            <el-option
              v-for="preset in customTransitionPresets"
              :key="`custom-${preset.value}`"
              :label="`${preset.label} (${preset.value})`"
              :value="preset.value"
            />
          </el-select>
        </el-form-item>
        <div v-if="selectedTransitionPresetValue === CUSTOM_TRANSITION_VALUE" class="transition-custom-panel">
          <div class="transition-custom-head">
            <div>
              <strong>自定义效果</strong>
              <p>填写 FFmpeg xfade 效果代码，保存后下次可直接复用。</p>
            </div>
            <el-button size="small" type="primary" plain @click="saveCustomTransitionPreset">保存效果</el-button>
          </div>
          <el-form-item label="效果名称">
            <el-input v-model="customTransitionDraft.label" maxlength="20" placeholder="例如：像素化转场" />
          </el-form-item>
          <el-form-item label="效果代码">
            <el-input
              v-model="customTransitionDraft.value"
              placeholder="例如：pixelize / radial / squeezeh"
              @input="handleCustomTransitionInput"
            />
          </el-form-item>
          <div class="transition-suggestion-list">
            <span class="transition-suggestion-title">快捷示例</span>
            <el-tag
              v-for="preset in suggestedCustomTransitions"
              :key="preset.value"
              class="transition-suggestion-tag"
              effect="plain"
              @click="useSuggestedCustomTransition(preset)"
            >
              {{ preset.label }} / {{ preset.value }}
            </el-tag>
          </div>
          <div v-if="customTransitionPresets.length" class="transition-saved-list">
            <span class="transition-suggestion-title">已保存</span>
            <div class="transition-saved-tags">
              <span v-for="preset in customTransitionPresets" :key="preset.value" class="transition-saved-chip">
                <button type="button" @click="applyTransitionPreset(preset)">{{ preset.label }}</button>
                <button type="button" class="remove" @click="removeCustomTransitionPreset(preset.value)">×</button>
              </span>
            </div>
          </div>
        </div>
...
const CUSTOM_TRANSITION_STORAGE_KEY = 'video-timeline-custom-transitions'
const CUSTOM_TRANSITION_VALUE = '__custom__'
const previewTransitionTypes = new Set([
  'fade',
  'fadeblack',
  'fadewhite',
  'slideleft',
...
const suggestedCustomTransitions: TransitionPreset[] = [
  { label: '像素化', value: 'pixelize', category: 'custom' },
  { label: '径向', value: 'radial', category: 'custom' },
  { label: '横向压缩', value: 'squeezeh', category: 'custom' },
  { label: '纵向压缩', value: 'squeezev', category: 'custom' },
]
const builtinTransitionPresets: TransitionPreset[] = [
  { label: '无转场', value: 'none', category: 'basic' },
  { label: '淡入淡出', value: 'fade', category: 'basic' },
  { label: '黑场过渡', value: 'fadeblack', category: 'basic' },
  { label: '白场过渡', value: 'fadewhite', category: 'basic' },
  { label: '灰场过渡', value: 'fadegrays', category: 'basic' },
  { label: '左滑', value: 'slideleft', category: 'motion' },
  { label: '右滑', value: 'slideright', category: 'motion' },
  { label: '上滑', value: 'slideup', category: 'motion' },
  { label: '下滑', value: 'slidedown', category: 'motion' },
  { label: '左擦除', value: 'wipeleft', category: 'motion' },
  { label: '右擦除', value: 'wiperight', category: 'motion' },
  { label: '上擦除', value: 'wipeup', category: 'motion' },
  { label: '下擦除', value: 'wipedown', category: 'motion' },
  { label: '圆形展开', value: 'circleopen', category: 'shape' },
  { label: '圆形收缩', value: 'circleclose', category: 'shape' },
  { label: '溶解', value: 'dissolve', category: 'shape' },
  { label: '距离拉开', value: 'distance', category: 'shape' },
  { label: '像素化', value: 'pixelize', category: 'shape' },
  { label: '水平打开', value: 'horzopen', category: 'shape' },
  { label: '水平关闭', value: 'horzclose', category: 'shape' },
  { label: '垂直打开', value: 'vertopen', category: 'shape' },
  { label: '垂直关闭', value: 'vertclose', category: 'shape' },
]
...
const customTransitionPresets = ref<TransitionPreset[]>([])
const selectedTransitionPresetValue = ref('fade')
const customTransitionDraft = ref({
  label: '',
  value: '',
})
...
const allTransitionPresets = computed(() => [...builtinTransitionPresets, ...customTransitionPresets.value])
...
// 预览相关
const sanitizeTransitionValue = (value: string) => {
  return value
    .trim()
    .toLowerCase()
    .replace(/\s+/g, '')
...
const isTransitionCodeValid = (value: string) => /^[a-z0-9_]{2,32}$/.test(sanitizeTransitionValue(value))

const loadCustomTransitionPresets = () => {
  try {
    const raw = localStorage.getItem(CUSTOM_TRANSITION_STORAGE_KEY)
    if (!raw) return
    const parsed = JSON.parse(raw)
    if (!Array.isArray(parsed)) return
    customTransitionPresets.value = parsed
      .filter((item) => typeof item?.label === 'string' && typeof item?.value === 'string')
      .map((item) => ({
        label: item.label.trim(),
        value: sanitizeTransitionValue(item.value),
        category: 'custom',
        custom: true,
      }))
      .filter((item) => item.label && isTransitionCodeValid(item.value))
  } catch (error) {
    console.warn('Failed to load custom transition presets:', error)
  }
}

const persistCustomTransitionPresets = () => {
  localStorage.setItem(CUSTOM_TRANSITION_STORAGE_KEY, JSON.stringify(customTransitionPresets.value))
}

const findTransitionPreset = (value: string) => {
  return allTransitionPresets.value.find((preset) => preset.value === value)
}

const applyTransitionPreset = (preset: TransitionPreset) => {
  selectedTransitionPresetValue.value = preset.custom ? CUSTOM_TRANSITION_VALUE : preset.value
  editingTransition.value.type = preset.value
  editingTransition.value.label = preset.label
  editingTransition.value.custom = !!preset.custom
  if (preset.custom) {
    customTransitionDraft.value = {
      label: preset.label,
      value: preset.value,
    }
  }
...
const useSuggestedCustomTransition = (preset: TransitionPreset) => {
  customTransitionDraft.value = {
    label: preset.label,
    value: preset.value,
  }
  selectedTransitionPresetValue.value = CUSTOM_TRANSITION_VALUE
  editingTransition.value.type = preset.value
  editingTransition.value.label = preset.label
  editingTransition.value.custom = true
}

const saveCustomTransitionPreset = () => {
  const label = customTransitionDraft.value.label.trim()
  const value = sanitizeTransitionValue(customTransitionDraft.value.value)

  if (!label) {
    ElMessage.warning('请先填写效果名称')
    return
  }
  if (!isTransitionCodeValid(value)) {
    ElMessage.warning('效果代码只能使用 2-32 位小写字母、数字或下划线')
    return
  }

  const existingIndex = customTransitionPresets.value.findIndex((preset) => preset.value === value)
  ...
  if (existingIndex >= 0) {
    customTransitionPresets.value.splice(existingIndex, 1, preset)
  } else {
    customTransitionPresets.value.unshift(preset)
  }

  persistCustomTransitionPresets()
  applyTransitionPreset(preset)
  ElMessage.success('自定义转场已保存')
}

const removeCustomTransitionPreset = (value: string) => {
  customTransitionPresets.value = customTransitionPresets.value.filter((preset) => preset.value !== value)
  persistCustomTransitionPresets()
  if (selectedTransitionPresetValue.value === value) {
    applyTransitionPreset(builtinTransitionPresets[1])
  }
}

const getTransitionPreviewType = (type: string) => {
  return previewTransitionTypes.has(type) ? type : 'fade'
}
...
const getTransitionLabel = (clip: TimelineClip) => {
  ...
  if (clip.transition.label) {
    return clip.transition.label
  }
  ...
  return labels[clip.transition.type] || clip.transition.type || '转场'
}

const handleTransitionPresetChange = (value: string) => {
  if (value === CUSTOM_TRANSITION_VALUE) {
    selectedTransitionPresetValue.value = CUSTOM_TRANSITION_VALUE
    editingTransition.value.custom = true
    editingTransition.value.type =
      sanitizeTransitionValue(customTransitionDraft.value.value) || editingTransition.value.type || 'fade'
    editingTransition.value.label = customTransitionDraft.value.label.trim() || editingTransition.value.label || '自定义效果'
    return
  }

  const preset = findTransitionPreset(value)
  if (preset) {
    applyTransitionPreset(preset)
  }
}

const handleCustomTransitionInput = () => {
  selectedTransitionPresetValue.value = CUSTOM_TRANSITION_VALUE
  editingTransition.value.type = sanitizeTransitionValue(customTransitionDraft.value.value) || 'fade'
  editingTransition.value.label = customTransitionDraft.value.label.trim() || '自定义效果'
  editingTransition.value.custom = true
}

const openTransitionDialog = (clip: TimelineClip) => {
  console.log('open transition dialog', {
    clip_id: clip.id,
  ...
  const currentType = clip.transition?.type || 'fade'
  const currentPreset = findTransitionPreset(currentType)
  const isCustomTransition = !!clip.transition?.custom || (!currentPreset && currentType !== 'none')
  editingTransition.value = {
    type: currentType,
    duration: clip.transition?.duration || 1.0,
    label: clip.transition?.label || currentPreset?.label || getTransitionLabel(clip),
    custom: isCustomTransition,
  }
  ...
  if (isCustomTransition) {
    ...
    selectedTransitionPresetValue.value = currentType
    customTransitionDraft.value = {
      label: '',
      value: '',
    }
...
const applyTransition = () => {
  ...
  if (!clip) {
    console.error('transition target clip not found:', editingTransitionClipId.value)
    transitionDialogVisible.value = false
    return
  }

  const isCustomTransition = selectedTransitionPresetValue.value === CUSTOM_TRANSITION_VALUE || editingTransition.value.custom
  const transitionType = isCustomTransition
    ? sanitizeTransitionValue(customTransitionDraft.value.value || editingTransition.value.type)
    : editingTransition.value.type

  if (transitionType !== 'none' && !findTransitionPreset(transitionType) && !isTransitionCodeValid(transitionType)) {
    ElMessage.warning('自定义转场代码只支持 2-32 位小写字母、数字或下划线')
    return
  }
  ...
  clip.transition = {
    ...
    label:
      transitionType === 'none'
        ? '无转场'
        : isCustomTransition
          ? customTransitionDraft.value.label.trim() || editingTransition.value.label || transitionType
          : findTransitionPreset(transitionType)?.label || editingTransition.value.label || transitionType,
    custom: transitionType !== 'none' && isCustomTransition,
  }
...
.video-timeline-editor {
  ...
  .transition-custom-panel {
    ...
    .transition-suggestion-list,
    .transition-saved-list {
      display: flex;
      flex-direction: column;
      gap: 8px;
    }

    .transition-suggestion-title {
      font-size: 12px;
      font-weight: 700;
      color: var(--text-secondary);
    }

    .transition-saved-tags {
      display: flex;
      flex-wrap: wrap;
      gap: 8px;
    }

    .transition-suggestion-tag {
      cursor: pointer;
    }

    .transition-saved-chip {
      display: inline-flex;
      align-items: center;
      gap: 6px;
      padding: 6px 10px;
