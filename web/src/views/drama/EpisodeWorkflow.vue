<template>
  <div class="page-container">
    <div class="content-wrapper animate-fade-in">
        ...
        >
          <div class="stage-body stage-body-fullscreen">
            ...
            <div v-else class="overview-section">
              ...
              <div class="action-buttons-inline">
                <el-button type="default" size="default" @click="startEditScript">
              ...
              >
                ...
                >
                  <template #default="{ row, $index }">
                    <div class="shot-operation-buttons">
                      <el-button
                        type="primary"
                        size="small"
      ...
      <!-- 编辑模式下，底部再提供一组保存/取消，避免按钮被视口遮挡 -->
      <div
        v-if="isEditingScript"
        class="action-buttons-inline"
...
const loadDramaData = async () => {
  try {
    ...
    // 同步当前章节文本内容到编辑框（非编辑状态下自动同步最新数据）
    if (!isEditingScript.value) {
      const ep = data.episodes?.find((item) => item.episode_number === episodeNumber);
...
const deleteShot = async (shot: any, index: number) => {
  try {
    await ElMessageBox.confirm(
      `确认删除镜头 ${shot?.storyboard_number || index + 1} 吗？`,
      $t("common.tip"),
      {
        type: "warning",
      },
    ...
    if (currentEpisode.value?.storyboards) {
      currentEpisode.value.storyboards.splice(index, 1);
      currentEpisode.value.storyboards = currentEpisode.value.storyboards.map(
        (item: any, orderIndex: number) => ({
          ...item,
          storyboard_number: item.storyboard_number || orderIndex + 1,
        }),
      );
    }
