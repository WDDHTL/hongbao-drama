...
func (s *VideoGenerationService) ProcessVideoGeneration(videoGenID uint) {
	...
	if drama.Style != "" && drama.Style != "realistic" {
		...
		if stylePrompt != "" {
			s.log.Infow("Added style prompt to video generation",
				"id", videoGenID,
				"style", drama.Style,
	...
	promptSections := make([]string, 0, 3)
	if stylePrompt != "" {
		promptSections = append(promptSections, stylePrompt)
	}
	if constraintPrompt != "" {
		promptSections = append(promptSections, constraintPrompt)
		s.log.Infow("Added constraint prompt to video generation",
			"id", videoGenID,
			"reference_mode", referenceMode,
			"constraint_prompt_length", len(constraintPrompt))
	}
	promptSections = append(promptSections, videoGen.Prompt)
...
func isVolcesVideoEndpoint(endpoint string) bool {
	normalized := strings.ToLower(endpoint)
	return strings.Contains(normalized, "/contents/generations/tasks")
}

func buildVideoClient(config *models.AIServiceConfig, fallbackProvider string, model string) (video.VideoClient, error) {
	endpoint := strings.TrimSpace(config.Endpoint)
	queryEndpoint := strings.TrimSpace(config.QueryEndpoint)
	actualProvider := normalizeVideoProvider(config.Provider)
	if actualProvider == "" {
		actualProvider = normalizeVideoProvider(fallbackProvider)
	}

	switch actualProvider {
	case "chatfire":
		return video.NewChatfireClient(config.BaseURL, config.APIKey, model, endpoint, queryEndpoint), nil
	case "doubao", "volcengine", "volces", "ark":
		return video.NewVolcesArkClient(config.BaseURL, config.APIKey, model, endpoint, queryEndpoint), nil
	case "openai", "sora", "openclaw", "custom_relay":
		switch {
		case isChatfireVideoEndpoint(endpoint) || isChatfireVideoEndpoint(queryEndpoint):
			return video.NewChatfireClient(config.BaseURL, config.APIKey, model, endpoint, queryEndpoint), nil
		case isVolcesVideoEndpoint(endpoint) || isVolcesVideoEndpoint(queryEndpoint):
			return video.NewVolcesArkClient(config.BaseURL, config.APIKey, model, endpoint, queryEndpoint), nil
		default:
			return video.NewOpenAISoraClient(config.BaseURL, config.APIKey, model, endpoint, queryEndpoint), nil
		}
	case "runway":
	...
	default:
		if isChatfireVideoEndpoint(endpoint) || isChatfireVideoEndpoint(queryEndpoint) {
			return video.NewChatfireClient(config.BaseURL, config.APIKey, model, endpoint, queryEndpoint), nil
		}
		if isVolcesVideoEndpoint(endpoint) || isVolcesVideoEndpoint(queryEndpoint) {
			return video.NewVolcesArkClient(config.BaseURL, config.APIKey, model, endpoint, queryEndpoint), nil
		}
		return video.NewOpenAISoraClient(config.BaseURL, config.APIKey, model, endpoint, queryEndpoint), nil
	}
...
func (s *VideoGenerationService) getVideoClient(provider string, modelName string) (video.VideoClient, error) {
	...
	config, err = s.aiService.GetPreferredConfig("video", provider, modelName)
	if err != nil {
	...
	return buildVideoClient(config, provider, model)
}
...
func (s *VideoGenerationService) BatchGenerateVideosForEpisode(episodeID string) ([]*models.VideoGeneration, error) {
	...
	storyboards := preferredStoryboardsForGeneration(episode.Storyboards)
