    ...
    <div class="content-wrapper animate-fade-in">
      ...
      >
        ...
        >
          ...
          <el-form-item :label="$t('drama.videoLanguage')" required>
            <div class="language-shell" role="radiogroup" :aria-label="$t('drama.videoLanguage')">
              <button
                v-for="item in languageOptions"
                :key="item.value"
                type="button"
                :class="['language-card', { selected: editForm.video_language === item.value }]"
                @click="editForm.video_language = item.value"
              >
          ...
          <el-form-item :label="$t('drama.style')" required>
            <div class="style-grid" role="radiogroup" aria-label="Drama styles">
              <button
                ...
                :key="item.value"
                type="button"
                :class="['style-card', { selected: editForm.style === item.value }]"
...
import { dramaStyleGallery } from "@/constants/dramaStyleGallery";
...
const editDrama = async (id: string) => {
  ...
  try {
    ...
    editForm.value = {
      ...
      video_language: drama.video_language || "zh-CN",
    };
  } catch (error: any) {
    ElMessage.error(error.message || "加载失败");
...
const saveEdit = async () => {
  ...
  try {
    await dramaAPI.update(editForm.value.id, {
      ...
      video_language: editForm.value.video_language,
    });
